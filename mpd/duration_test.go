...
func TestParseDuration(t *testing.T) {
	in := map[string]float64{
		...
		"P0M0DT0H3M30.000S":   (3*time.Minute + 30*time.Second).Seconds(),
		"P0Y0M0DT0H3M30.000S": (3*time.Minute + 30*time.Second).Seconds(),
	}
	for ins, ex := range in {
...
func TestParseBadDurations(t *testing.T) {
	in := map[string]string{
		"P20M":   `duration must be in the format: P[nD][T[nH][nM][nS]]`, // We don't allow non-zero Months (doesn't make sense when converting to duration)
		"P20Y":   `duration must be in the format: P[nD][T[nH][nM][nS]]`, // We don't allow non-zero Years (doesn't make sense when converting to duration)
